...
class Student:
    def __init__(self):
        self.path=r"student_registration_task_python/oops/datastore.json"
        if os.path.exists(self.path):
            with open(self.path,'r') as file:
                
                try:
                    self.list_data= json.load(file)   
    ...
    def student_registration(self):
        ...
        while True:
            ...
            ask_sumbit=input('do your sumbit data(yes/no): ')
            if ask_sumbit =='yes':
                self.list_data.append(self.student_data)
                with open(self.path,'w') as file:
                    json.dump(self.list_data,file,indent=4)
                
                print('Your Registration is complete successfully!') 
            else:
                print('your data is not submit!')
